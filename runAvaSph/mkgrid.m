function grid = mkgrid(file, nhl, ds, savefile, plots)
    """ Author : Dieter Issler (NGI), 2021-07-14
    
        Octave function that generates a grid of cells with uniform oblique 
        length along a 3D polyline read from an input file. The resulting grid
        does not go exactly through the polyline points unless they coincide
        with a node.
        
        mkgrid also calculates slope angle and curvature at the nodes and
        linearly interpolates channel width, snow-cover proprties and initial
        conditions read from the input file. The resulting array can optionally
        be stored in .mat file and selected quantities plotted.
        
        The code is based on cells_divisor3.m by Amaury Bélières-Frendo,
        inspired by the author's code polygrid.0.6.c.
        
        Usage:
        grid = mkgrid(<input>, <#header>, <cell length>, <output>, <plot list> 
      
    Parameters
      ----------
      <input>: Name of input file. ASCII file with 6 header lines
               ...
               velocity u0 (m/s).
      <#header>: Number of header lines to skip in input file.
      <cell length>: Oblique length of grid cells (m), must be smaller than
                     shortest input segment length.
      <output file>: Name of .mat file to save arr to. Can be '' to omit saving.
    ...
    Returns
      ...
      Return: Array of size n_pts × 12, with n_pts the number of grid cells and
              components x, y, z, l (horizontal length along path), w (interpol-
              ated width), cos θ, sin θ, curvature κ (1/m), tau_c (interpol-
              ated), b0  (interpolated), h0 (interpolated), u0 (interpolated).
    """

    global nf  N  n_points  P  W  fric  tau_c  b0  h0  u0;

    [P, W, fric, tau_c, b0, h0, u0, N, nf] = read_input(file, nhl);

    % Determine the segment lengths and the total number of cells:
    [n_points, L_seg] = segments(ds, N, P);
    if n_points < 0                     % segments() has thrown an error
      return;
    endif

    % Allocate some arrays:
    grid = zeros(n_points, 12+nf);      % 1: X, 2: Y, 3: Z, 4: L, 5: W,
                                        % 6: cos θ, 7: sin θ, 8: kappa,
                                        % 9: tau_c, 10: b0, 11: h0, 12: u0
                                        % 13ff: μ, k, ...
    loc  = zeros(n_points, 2);          % [segment #, fractional dist. in seg.]

    % Compute unit tangent vectors along each segment & angles between segments:
    ...
    grid(1,:) = [P(1,:), 0.0, W(1), cos_theta(1), sin_theta(1), 0.0, ...
                 tau_c(1), b0(1), h0(1), u0(1), fric(1,:)];

    loc(1,:) = [1, 0.0];

    % Divide the polyline into cells of length ds
    [n_points, grid, loc] ...
        = cell_division(n_points, ds, L_seg, tvec, P, cos_beta, sin_beta, ...
                        cos_theta, sin_theta, grid, loc);

    % Interpolate the input data to the grid and compute slope, curvature
    grid = interpolate(loc, L_seg, grid, W, fric, tau_c, b0, h0, u0);
    [grid(:,6), grid(:,7), grid(:,8)] = diff_geom(ds, n_points, grid(:,3));
...
function [P, W, fric, tau_c, b0, h0, u0, N, nfric] = read_input(fil, nhl)

    Readfile = importdata(fil, ' ', nhl);
    ...
    fric  = Readfile.data(:,9:end);     % Friction coefficients

    N = size(P)(1);                     % Number of input points
    nfric = size(fric)(2);              % Number of friction coefficients
...
% Divide the input polyline into cells of length h

function [np, grid, loc] = cell_division(np, h, L_seg, tvec, P, cos_b, ...
                                         sin_b, cos_t, sin_t, grid, loc)
...
% Interpolate path width, snow-cover properties and friction coefficients

    % Linear interpolation between input points for W, tau_c, b0, and f.
...
function grid = interpolate(loc, L_seg, grid, W, fric, tau_c, b0, h0, u0)

    global N  n_points  nf  ds;

    % Linear interpolation between input points for W, tau_c, b0 and fric:
    grid(:,[5 9 10 13:12+nf]) ...
        = [W(loc(:,1))  tau_c(loc(:,1))  b0(loc(:,1))  fric(loc(:,1),:)] ...
              .* (1.0-loc(:,2)) ...
          + [W(min(loc(:,1)+1, N))  tau_c(min(loc(:,1)+1, N)) ...
             b0(min(loc(:,1)+1, N))  fric(min(loc(:,1)+1, N),:)] .* loc(:,2);
    ...
    frac_lo = loc(j_min,2) * L_seg(i_min) / ds;
    frac_hi = (1.0-loc(j_max,2)) * L_seg(i_max-1) / ds;
...
function postprocess(grid, filename, plotlist)
    ...
    global N  P  W  tau_c  tau_c  b0  h0  u0  fric;
    ...
    for i = 1:size(plotlist)(1)
        ...
        switch(var)
            ...
            case 'u0'                   % Initial velocity
                ...
                asp = 'auto';
            case 'fric'                 % Friction coefficients
                res_v = grid(:,13:end);
                inp_v = fric;
                yax = 'Friction coefficients (―)';
                asp = 'auto';
            otherwise
